        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 md:gap-4">
          {services.map((svc, idx) => (
            <motion.button
              ...
              className={`text-left rounded-xl border transition-all duration-300 relative group overflow-hidden ${
                activeId === svc.id
            ...
            >
              ...
              <div className="relative w-full h-24 overflow-hidden">
                ...
                {/* Icon overlay */}
                <div className={`absolute bottom-2 left-2 w-7 h-7 rounded-lg flex items-center justify-center transition-colors ${
                  activeId === svc.id ? 'bg-primary text-white' : 'bg-black/50 text-primary backdrop-blur-sm'
                }`}>
                  <svc.icon size={14} />
                </div>
                {activeId === svc.id && (
              ...
              <div className="p-2 md:p-3">
                <h3 className="text-xs font-secondary font-bold text-white leading-tight mb-0.5">{svc.title}</h3>
                <p className="text-[10px] text-white/35 line-clamp-1 leading-snug">{svc.desc}</p>
              </div>
            </motion.button>
          ))}
        </div>
          ...
          {active && (
            ...
            >
              ...
              <div className="grid lg:grid-cols-2">
                ...
                {/* RIGHT — Image (smaller, contained) */}
                <div className="flex items-center justify-center p-6 md:p-10 border-b lg:border-b-0 lg:border-l border-white/5 bg-white/[0.01]">
                  <div className="relative w-full max-w-sm rounded-2xl overflow-hidden shadow-[0_0_40px_rgba(62,160,255,0.15)] border border-white/10">
                    <motion.img
                      key={active.id + '-img'}
                      src={active.image}
                      alt={active.title}
                    ...
                    {/* Floating badge */}
                    <div className="absolute bottom-4 left-4 px-3 py-1.5 rounded-full bg-primary/20 border border-primary/40 backdrop-blur-md">
                      <span className="text-primary text-[11px] font-bold tracking-wider uppercase">{active.title}</span>
                    </div>
                  </div>
                </div>
              </div>
